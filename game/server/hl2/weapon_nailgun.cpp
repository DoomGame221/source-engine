...
private:
        ...
        bool m_bIsBurstMode; // Ê¶Ò¹Ð¡ÒÃÂÔ§ burst fire
        int m_nBurstShotsRemaining; // ¨Ó¹Ç¹¡ÃÐÊØ¹·ÕèàËÅ×Íã¹ burst fire
...
void CWeaponNailgun::PrimaryAttack(void)
{
        ...
        if (m_bIsBurstMode)
        {
                ...
                {
                        m_nBurstShotsRemaining = 3; // ¨Ó¹Ç¹¡ÃÐÊØ¹ã¹ burst fire
                }
                ...
                if (m_nBurstShotsRemaining > 0)
                {
                        m_flNextPrimaryAttack = gpGlobals->curtime + 0.1f; // àÇÅÒ delay ÃÐËÇèÒ§ burst shots
                }
                else
                {
...
void CWeaponNailgun::ItemPostFrame(void)
{
        ...
        // µÃÇ¨ÊÍº¡ÒÃ¡´¤ÅÔ¡¢ÇÒà¾×èÍÊÅÑºâËÁ´
        if (pOwner->m_nButtons & IN_ATTACK2)
        {
                if (gpGlobals->curtime - m_flLastModeSwitchTime > 0.5f) // »éÍ§¡Ñ¹¡ÒÃÊÅÑºâËÁ´ºèÍÂà¡Ô¹ä»
                {
                        m_bIsBurstMode = !m_bIsBurstMode; // ÊÅÑºâËÁ´¡ÒÃÂÔ§

                        // áÊ´§¢éÍ¤ÇÒÁ·Õè¡ÅÒ§Ë¹éÒ¨Í
                        if (m_bIsBurstMode)
                        {
                        ...
                        m_flLastModeSwitchTime = gpGlobals->curtime; // ÍÑ¾à´µàÇÅÒàÁ×èÍÊÅÑºâËÁ´
                }
        }
